...
namespace OxyPlot.Wpf
{
    ...
    public class CanvasRenderContext : RenderContextBase
    {
        ...
        /// The image cache
        /// </summary>
		private readonly Dictionary<OxyImage, TextureSource> imageCache = new Dictionary<OxyImage, TextureSource>();
     ...
     //   public bool RendersToScreen { get; set; }
        ...
        public override void DrawEllipse(OxyRect rect, OxyColor fill, OxyColor stroke, double thickness)
        {
			...
			e.Width = (float)rect.Width;
			e.Height = (float)rect.Height;
			Canvas.SetLeft(e, (float)rect.Left);
			Canvas.SetTop(e, (float)rect.Top);
        }
            ...
            bool aliased)
        {
			...
			// @CONV: When noesis adds support for Polyline this code can be reverted to the OxyPlot version
			for(int i = 0;i< points.Count-1;i++)
			{
	            var e = this.CreateAndAdd<Line>();
				e.X1 = (float)points[i].X;
				e.Y1 = (float)points[i].Y;

				e.X2 = (float)points[i+1].X;
				e.Y2 = (float)points[i+1].Y;
        ...
        public override void DrawLineSegments(
            ...
            bool aliased)
        {
  			// @CONV: Noesis only supports StreamGeometry
...
//            }
            ...
            bool aliased)
        {
            ...
            this.SetStroke(e, stroke, thickness, lineJoin, dashArray, 0, false);
			...
			context.BeginFigure(new Noesis.Point((float)points[0].X, (float)points[0].Y), true);
			// @CONV: When noesis adds support for Polyline this code can be reverted to the OxyPlot version
			for(int i = 1;i< points.Count;i++)
			{
				context.LineTo(new Noesis.Point((float)points[i].X, (float)points[i].Y));	
			}
			context.Close();

			e.Data = streamGeometry;
            ...
            OxySize? maxSize)
        {
            ...
            {
                tb.Measure(new Noesis.Size(1000, 1000));
                var size = tb.DesiredSize;
                if (maxSize != null)
                {
                    if (size.Width > maxSize.Value.Width + 1e-3)
                    {
						size.Width = (float)Math.Max(maxSize.Value.Width, 0);
                    }

                    if (size.Height > maxSize.Value.Height + 1e-3)
                    {
						size.Height = (float)Math.Max(maxSize.Value.Height, 0);
                    }
			...
			transform.Children.Add(new TranslateTransform((float)dx, (float)dy));
            if (Math.Abs(rotate) > double.Epsilon)
            {
				transform.Children.Add(new RotateTransform((float)rotate));
            }

			transform.Children.Add(new TranslateTransform((float)p.X, (float)p.Y));
        ...
        //    TextOptions.SetTextFormattingMode(tb, this.TextFormattingMode);
			...
			return new OxySize((double)tb.DesiredSize.Width, (double)tb.DesiredSize.Height);
        }
            ...
            bool interpolate)
        {
           ...
           //     bitmapChain = new CroppedBitmap(bitmapChain, new Int32Rect((int)srcX, (int)srcY, (int)srcWidth, (int)srcHeight));
			...
			image.Width = (float)destWidth;
			image.Height = (float)destHeight;
			...
			Canvas.SetLeft(image, (float)destX);
			Canvas.SetTop(image, (float)destY);
...
//            }
        ...
        private static OxySize MeasureTextSize(Font glyphTypeface, double sizeInEm, string s)
        {
            double width = 0;
            double lineWidth = 0;
            int lines = 0;
            foreach (char ch in s)
            {
				...
				double advanceWidth = glyphTypeface.GetGlyphAdvance(ch);
                //= glyphTypeface.AdvanceWidths[glyph];
...
//                        new Noesis.Rect(
//							this.clip.Value.X - (float)clipOffsetX,
//							this.clip.Value.Y - (float)clipOffsetY,
...
//            }
            ...
            bool aliased)
        {
            ...
            for (int i = 0; i + 1 < points.Count; i += 2)
            {
				...
				streamGeometryContext.BeginFigure(new Noesis.Point((float)points[i].X, (float)points[i].Y), false);
				streamGeometryContext.LineTo(new Noesis.Point((float)points[i + 1].X, (float)points[i + 1].Y));
            ...
            bool aliased = false)
        {
            if (!stroke.IsUndefined() && thickness > 0)
            {
                ...
                if (dashArray != null)
                {
					// @CONV: NoesisGUI expects a string - @TODO: Convert double collection to string
//                    shape.StrokeDashArray = new DoubleCollection(dashArray);
					shape.StrokeDashOffset = (float)dashOffset;
                }
            }
       ...
       /*     // balance the number of points per polyline and the number of polylines
        ...
        private Noesis.Point ToPoint(ScreenPoint pt)
        {
			return new Noesis.Point((float)pt.X, (float)pt.Y);
        }
        ...
        private Noesis.Point ToPixelAlignedPoint(ScreenPoint pt)
        {
			...
			return new Noesis.Point((float)(0.5 + (int)pt.X), (float)(0.5 + (int)pt.Y));
        }
        ...
        private Noesis.Rect ToRect(OxyRect r)
        {
			return new Noesis.Rect((float)r.Left, (float)r.Top, (float)r.Width, (float)r.Height);
        }
        ...
        private Noesis.Rect ToPixelAlignedRect(OxyRect r)
        {
			...
			return new Noesis.Rect((float)x, (float)y, (float)(ri - x), (float)(bo - y) );
        }
        ...
        /// <returns>A <see cref="Point" />.</returns>
        private Noesis.Point ToPoint(ScreenPoint pt, bool aliased)
        {
        ...
        {
			List<Noesis.Point> coll = new PointCollection();
			foreach(var p in points)
			{
				if(aliased)
					coll.Add(ToPixelAlignedPoint(p));
				else
					coll.Add(ToPoint(p));
			}
			return coll;
